...
receita <- recipe(Response ~ ., treinamento) %>% # define a receita, com a variavel resposta e os dados de treinamento
  ...
  step_mutate(Response = as.factor(Response)) # transformando variável resposta em fator
...
Transformando variável "Response" em Factor
...
fit_glm <- logistic_reg() %>% # define um modelo de regressao logistica
  ...
  fit(Response ~ ., treinamento_proc) # executa o modelo e estima os parametros
