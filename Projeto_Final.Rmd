<<<<<<< HEAD
---
title: "Projeto Final AEM II"
author: "André Dambry, Mainara Cardoso, Tiago Pardo"
date: "`r Sys.Date()`"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## Objetivo:

### 1) Utilizar métodos de aprendizagem supervisionada (regressão ou classificação)

### 2) Realização de análise de um problema não supervisionado

## Base de dados:

### Marketing Analytics

<https://www.kaggle.com/datasets/jackdaoud/marketing-data/data?select=dictionary.png>

A base de dados contém informações de 2206 clientes de uma empresa. Os dados são relacionados à:

-   Perfil dos clientes

-   Produtos

-   Campanhas bem / mau sucedidas

-   Performance do canal

# Proposta de Valor

Através dos modelos supervisionados, nossa proposta é dentificar chance de clientes aderirem às campanhas.

Com isto feito, nossa proposta, através do método KMeans será realizar o agrupamento de clientes. Isso é útil para que a realização de segmentação de público em campanhas de marketing.

## Modelos supervisionados:

Tipo de Problema: Classificação

Modelos:

-   Regressão Logística

-   Floresta Aleatória

-   Boosting

-   Lasso

-   Ridge

-   Rede Neural

## Modelos não supervisionados:

-   KMeans

# Bibliotecas

```{r}
#install.packages("summarytools")
#install.packages("reticulate")
#install.packages("keras")
#install.packages("imbalance")
#install.packages("DMwR")
#install.packages("Rtools")
#install.packages( "Path/To/DMwR_0.4.1.tar.gz", repos=NULL, type="source" )
#install.packages("caret")
```

```{r}
library (readr)
library (dplyr)
library (tidyr)
library (tidyverse)
library (skimr)
library (ggplot2)
library (summarytools)
library (tidymodels)
library (ISLR)
library (doParallel)
library (glmnet)
library (ranger)
library (parsnip)
library (vip)
library (keras)
library (corrplot)
library (imbalance)
library (tensorflow) 
library (caret)
library (rlang)
library (pROC)
library (dials)
```

# Importação de dados

```{r}
urlfile <- "https://raw.githubusercontent.com/nailson/ifood-data-business-analyst-test/master/ifood_df.csv"
dados <- read_csv(url(urlfile))
```

# Geração de metadados

```{r}
# metadata <- dados  %>%
#   lapply(type_sum) %>%
#   as_tibble() %>%
#   pivot_longer(cols = 1:ncol(dados),
#                names_to = "Coluna",
#                values_to = "Tipo") %>%
#   inner_join(
#     dados %>%
#       summarise(across(everything(), ~sum(is.na(.)))) %>%
#       pivot_longer(cols = 1:ncol(dados),
#                    names_to = "Coluna",
#                    values_to = "Total NA")
#   )
# 
# metadata
```

# 1. Análise Descritiva

```{r}
summary(dados)
```

```{r}
skim(dados)
```

## 1.1. Verificando ausência de valores

```{r}
colSums(is.na(dados))
```

A base de dados não possui valores ausentes.

## 1.2. Verificando o tipo de dados de cada coluna.

```{r}
sapply(dados, class)
```

Observa-se que todos as colunas são do tipo numérica. Isso ocorre porque as variáveis categóricas, como estado civil (marital) e nível de educação (education), já estão convertidas para dummy.

## 1.3. Verificando valores negativos.

```{r}
sapply(dados, function(x) sum(x < 0))

```

A variável "MntRegularProds", que representa os gastos com produtos regulares, é a única que apresenta valores negativos.

## 1.4. Verificando colunas com valores únicos.

```{r}
col_valores_unicos <- sapply(dados, function(x) length(unique(x)) == 1)
names(dados)[col_valores_unicos]
```

As colunas "Z_CostContact" e "Z_Revenue", que representam, respectivamente, o custo do contato e o valor da receita, possuem um único valor em todas as observações.

Assim, pontos a serem destacados sobre a análise inicial dos dados.

1.  Ausência de valores vazios.
2.  Tipo dos dados estão corretos.
3.  Variáveis categóricas estão previamente tratadas como dummies.
4.  A coluna "MntRegularProds" possui valores negativos.
5.  As colunas "Z_CostContact" e "Z_Revenue" possuem um único valor em todas as observações.

## 1.5. Análise de outliers

Para melhor funcionamento dos modelos, verificaremos a existência de outliers nas colunas com valores numéricos.

Obs: Não consideramos colunas com valores numéricos binários ou que não façam sentido de terem seus outliers analisados.

Verificando colunas com dados binários.

```{r}
colunas_binarias <- sapply(dados, function(x) length(unique(x)) == 2)
names(dados)[colunas_binarias]
```

```{r}
# Selecionar colunas não-binárias
colunas_nao_binarias <- names(dados)[!colunas_binarias]

# Cálculo de outliers

for (coluna in colunas_nao_binarias) {
  q1 <- quantile(dados[[coluna]], 0.25)
  q3 <- quantile(dados[[coluna]], 0.75)
  q95 <- quantile(dados[[coluna]], 0.95)
  iqr <- q3 - q1
  median <- median(dados[[coluna]])
  outlier_limit <- median + 1.5 * iqr
  # Count the number of values above the limit
  outlier_count <- sum(dados[[coluna]] > outlier_limit)
  cat(sprintf('%20s | median+1.5xiqr: %8.2f | 95quantile: %8.2f | outliers: %d\n', 
              coluna, outlier_limit, q95, outlier_count))
}
```


```{r}
# Configuração de layout para exibir 4x5 boxplots
par(mfrow = c(4, 5), mar = c(2, 2, 2, 2))

#Plot dos boxplots
for (coluna in colunas_nao_binarias) {
  boxplot(dados[[coluna]], main = coluna, coluna = "lightblue", border = "black", notch = TRUE)
}
```

Dados que temos outliers nos dados, faremos a normalização dos dados na etapa de utilização dos TidyModels para os modelos aplicados.

# 2. Tratamento dos dados

## 2.1 Limpeza

Na análise realizada, indentificamos que a variável "MntRegularProds", que indica o valor das compras efetuadas na plataforma, possui 3 valores negativos. Considerando que os valores das compras não podem ser negativos por natureza e que o número de observações com essa característica é pequeno, optamos por remover esses dados da base.

```{r}
# filtrando valores negativos da base 
dados <- dados %>%             
  filter (dados$MntRegularProds > 0)
```

As colunas "Z_CostContact" e "Z_Revenue", que representam, respectivamente, o custo do contato e o valor da receita, possuem um único valor em todas as observações. Logo, essas colunas não contribuem com informações úteis para aprimorar a precisão dos modelos preditivos ou da segmentação dos clientes. Portanto, estas variáveis serão excluídas da base.

```{r}
col_rem <- c("Z_CostContact", "Z_Revenue")  
dados <- dados %>% select(-one_of(col_rem))
```

```{r}
dados
```

# 3. Análise Exploratória de Dados

## 3.1. Análise Univariada

### 3.1.1. Variáveis Numéricas

#### 3.1.1.1. Variáveis de Campanhas de Marketing

```{r}
# Contagem de frequências para cada campanha
cat("AcceptedCmp1\n")
print(table(dados$AcceptedCmp1))

cat("\nAcceptedCmp2\n")
print(table(dados$AcceptedCmp2))

cat("\nAcceptedCmp3\n")
print(table(dados$AcceptedCmp3))

cat("\nAcceptedCmp4\n")
print(table(dados$AcceptedCmp4))

cat("\nAcceptedCmp5\n")
print(table(dados$AcceptedCmp5))

cat("\nResponse\n")
print(table(dados$Response))

```


verificando quantos clientes aderiram a pelo menos uma campanha
```{r}
adesao_campanhas <- dados %>% 
  filter(AcceptedCmp1 == 1 | AcceptedCmp2 == 1 | AcceptedCmp3 == 1 | AcceptedCmp4 == 1 | AcceptedCmp5 == 1)
```

```{r}
nrow(adesao_campanhas)
```


#### 3.1.1.2. Variável Resposta

```{r}
# Análise da variável 'Response'
ggplot(dados, aes(x = Response)) +
  geom_bar() +
  labs(title = 'Distribuição da Variável Response', x = 'Response', y = 'Contagem')

```

Conforme observado, em todas as campanhas realizadas, o percentual de aceitação da campanha é muito inferior ao percentual de não aceitação, inclusive no caso da variável "Response". Esse desbalanceamento dos dados pode ocasionar problemas na previsão do modelo, visto que o modelo pode apresentar um erro maior na classificação da classe minoritária e, ainda assim, apresentar uma alta acurácia do modelo.

Dessa forma, é importante entender o efeito do falso positivo e do falso negativo para o cenário analisado. Como o objetivo da nossa previsão é decidir se compensa enviar ou não a sexta campanha de marketing para um cliente, então as consequências de uma classificação errada para o cliente são:

-   Falso negativo, que seria uma previsão errada de que um cliente não vai aceitar a campanha, é um custo de oportunidade.
-   Falso positivo, que seria uma previsão errada de que um cliente vai aceitar a campanha, implica no custo do envio da proposta de marketing.

### 3.1.2. Variáveis Categóricas

Como as variáveis categóricas estão no formato de one-hot encoding, vamos retorná-las para suas configurações originais para analisá-las.

#### 3.1.2.1 Criando coluna maritial_status

```{r}
# Copiando a tabela original
aux <- dados
```

```{r}
one_hot_cols <- c("marital_Divorced", "marital_Married", "marital_Single", "marital_Together", "marital_Widow")

aux$marital_status <- colnames(aux[one_hot_cols])[max.col(aux[one_hot_cols],"first")] 
```

```{r}
#removendo colunas codificadas da base
aux <- aux[, !(names(aux) %in% one_hot_cols)]

```

#### 3.1.2.2. Criando coluna Education_level

```{r}
one_hot_cols <- c("education_2n Cycle", "education_Basic", "education_Graduation", "education_Master", "education_PhD")

aux$education_level <- colnames(aux[one_hot_cols])[max.col(aux[one_hot_cols],"first")] 
```

```{r}
aux <- aux[, !(names(aux) %in% one_hot_cols)]

```

#### 3.1.2.3 Tabela de frequência

marital_status

```{r}
tabela_frequencia_absoluta <- table(aux$marital_status)

tabela_frequencia_relativa <- prop.table(tabela_frequencia_absoluta)*100

tabela_frequencia <- cbind('Frequencia Absoluta' = tabela_frequencia_absoluta, 'Frequencia Relativa' = tabela_frequencia_relativa)

print(tabela_frequencia)

```
```{r}
barplot(tabela_frequencia_absoluta, main = "Distribuição de marital_status", xlab = "marital_status", ylab = "Frequência", cex.names = 0.6)
```

```{r}
tabela_frequencia_absoluta <- table(aux$education_level)

tabela_frequencia_relativa <- prop.table(tabela_frequencia_absoluta)*100

tabela_frequencia <- cbind('Frequencia Absoluta' = tabela_frequencia_absoluta, 'Frequencia Relativa' = tabela_frequencia_relativa)

print(tabela_frequencia)

```

```{r}
barplot(tabela_frequencia_absoluta, main = "Distribuição de education_level", xlab = "education_level", ylab = "Frequência",cex.names = 0.6)
```



## 3.2. Análise Bivariada

### 3.2.1. Variáveis Numéricas



```{r}
colunas_selecionadas <- c('Income', 'Recency', 'MntWines', 'MntMeatProducts', 'MntFishProducts', 'MntSweetProducts')

for (col in colunas_selecionadas) {
  p <- ggplot(dados, aes(x = as.factor(Response), y = !!sym(col))) +
    geom_boxplot() +
    labs(title = paste('Relação entre', col, 'e Response'))
  print(p)
}
```


```{r}

# Convertendo 'Response' para fator
dados$Response <- as.factor(dados$Response)

# Gráficos de dispersão
par_variaveis <- list(c('MntMeatProducts', 'MntFishProducts'), c('MntGoldProds', 'MntSweetProducts'), c('Recency', 'MntWines'))
for (par in par_variaveis) {
  p <- ggplot(dados, aes(x = !!sym(par[1]), y = !!sym(par[2]), color = Response)) +
    geom_point() +
    labs(title = paste('Relação entre', par[1], 'e', par[2], 'por Response'), x = par[1], y = par[2])
  print(p)
}



```


### Valor gasto por tipo de produtos por clientes que aderiram a somente uma campanha

```{r}
adesao_apenas_cmp1 <- dados %>% 
  filter (dados$AcceptedCmp1 == 1 & dados$AcceptedCmp2 == 0 & dados$AcceptedCmp3 == 0 & dados$AcceptedCmp4 == 0 & dados$AcceptedCmp5 == 0 ) %>% 
  select(-MntTotal)
```

Verificando quantidade de clientes que só aderiram a campanha 1
```{r}
nrow(adesao_apenas_cmp1)
```

```{r}

# Calcular a razão para todos os produtos
razoes <- dados %>%
  select(-MntTotal) %>% 
  summarise(across(starts_with("Mnt"), sum)) / 
  adesao_apenas_cmp1 %>% 
  summarise(across(starts_with("Mnt"), sum))

# Transformar os dados para o formato longo
razoes_long <- razoes %>%
  pivot_longer(cols = starts_with("Mnt"), names_to = "Produto", values_to = "Razao")

# Criar um gráfico de barras
ggplot(razoes_long, aes(x = Produto, y = Razao, fill = Produto)) +
  geom_bar(stat = "identity") +
  labs(title = "Razão do valor gasto por clientes exclusivos da 1ª campanha pelo total",
       x = "Produto",
       y = "Razão") +
  theme_minimal() + 
  theme(axis.text.x = element_blank())
```


#### 3.3.1.2
```{r}
adesao_apenas_cmp2 <- dados %>% 
  filter (dados$AcceptedCmp1 == 0 & dados$AcceptedCmp2 == 1 & dados$AcceptedCmp3 == 0 & dados$AcceptedCmp4 == 0 & dados$AcceptedCmp5 == 0 ) %>% 
  select(-MntTotal)
```

Verificando quantidade de clientes que só aderiram a campanha 2
```{r}
nrow(adesao_apenas_cmp2)
```

```{r}

# Calcular a razão para todos os produtos
razoes <- dados %>%
  select(-MntTotal) %>% 
  summarise(across(starts_with("Mnt"), sum)) / 
  adesao_apenas_cmp2 %>% 
  summarise(across(starts_with("Mnt"), sum))

# Transformar os dados para o formato longo
razoes_long <- razoes %>%
  pivot_longer(cols = starts_with("Mnt"), names_to = "Produto", values_to = "Razao")

# Criar um gráfico de barras
ggplot(razoes_long, aes(x = Produto, y = Razao, fill = Produto)) +
  geom_bar(stat = "identity") +
  labs(title = "Razão do valor gasto por clientes exclusivos da 2ª campanha pelo total",
       x = "Produto",
       y = "Razão") +
  theme_minimal() + 
  theme(axis.text.x = element_blank())
```


#### 3.3.1.3.

```{r}
adesao_apenas_cmp3 <- dados %>% 
  filter (dados$AcceptedCmp1 == 0 & dados$AcceptedCmp2 == 0 & dados$AcceptedCmp3 == 1 & dados$AcceptedCmp4 == 0 & dados$AcceptedCmp5 == 0 ) %>% 
  select(-MntTotal)
```

Verificando quantidade de clientes que só aderiram a campanha 3
```{r}
nrow(adesao_apenas_cmp3)
```

```{r}

# Calcular a razão para todos os produtos
razoes <- dados %>%
  select(-MntTotal) %>% 
  summarise(across(starts_with("Mnt"), sum)) / 
  adesao_apenas_cmp3 %>% 
  summarise(across(starts_with("Mnt"), sum))

# Transformar os dados para o formato longo
razoes_long <- razoes %>%
  pivot_longer(cols = starts_with("Mnt"), names_to = "Produto", values_to = "Razao")

# Criar um gráfico de barras
ggplot(razoes_long, aes(x = Produto, y = Razao, fill = Produto)) +
  geom_bar(stat = "identity") +
  labs(title = "Razão do valor gasto por clientes exclusivos da 3ª campanha pelo total",
       x = "Produto",
       y = "Razão") +
  theme_minimal() + 
  theme(axis.text.x = element_blank())
```


#### 3.3.1.4.
```{r}
adesao_apenas_cmp4 <- dados %>% 
  filter (dados$AcceptedCmp1 == 0 & dados$AcceptedCmp2 == 0 & dados$AcceptedCmp3 == 0 & dados$AcceptedCmp4 == 1 & dados$AcceptedCmp5 == 0 ) %>% 
  select(-MntTotal)
```

Verificando quantidade de clientes que só aderiram a campanha 4
```{r}
nrow(adesao_apenas_cmp4)
```


```{r}

# Calcular a razão para todos os produtos
razoes <- dados %>%
  select(-MntTotal) %>%
  summarise(across(starts_with("Mnt"), sum)) / 
  adesao_apenas_cmp4 %>%
  summarise(across(starts_with("Mnt"), sum))

# Transformar os dados para o formato longo
razoes_long <- razoes %>%
  pivot_longer(cols = starts_with("Mnt"), names_to = "Produto", values_to = "Razao")

# Criar um gráfico de barras
ggplot(razoes_long, aes(x = Produto, y = Razao, fill = Produto)) +
  geom_bar(stat = "identity") +
  labs(title = "Razão do valor gasto por clientes exclusivos da 4ª campanha pelo total",
       x = "Produto",
       y = "Razão") +
  theme_minimal() + 
  theme(axis.text.x = element_blank())

```



#### 3.3.1.5.
```{r}
adesao_apenas_cmp5 <- dados %>% 
  filter (dados$AcceptedCmp1 == 0 & dados$AcceptedCmp2 == 0 & dados$AcceptedCmp3 == 0 & dados$AcceptedCmp4 == 0 & dados$AcceptedCmp5 == 1 ) %>% 
  select(-MntTotal)
```

Verificando quantidade de clientes que só aderiram a campanha 5
```{r}
nrow(adesao_apenas_cmp5)
```


```{r}

# Calcular a razão para todos os produtos
razoes <- dados %>%
  select(-MntTotal) %>%
  summarise(across(starts_with("Mnt"), sum)) / 
  adesao_apenas_cmp5 %>%
  summarise(across(starts_with("Mnt"), sum))

# Transformar os dados para o formato longo
razoes_long <- razoes %>%
  pivot_longer(cols = starts_with("Mnt"), names_to = "Produto", values_to = "Razao")

# Criar um gráfico de barras
ggplot(razoes_long, aes(x = Produto, y = Razao, fill = Produto)) +
  geom_bar(stat = "identity") +
  labs(title = "Razão do valor gasto por clientes exclusivos da 5ª campanha pelo total",
       x = "Produto",
       y = "Razão") +
  theme_minimal() + 
  theme(axis.text.x = element_blank())

```


## 2. Separação entre treino e teste

```{r}
split <- initial_split(dados, prop = 0.7, strata = "Response")

treinamento <- training(split)
teste <- testing(split)
```

# 3. Modelagem Estatística

Utilizaremos Tidy Models para os modelos que o comportam

```{r}
receita <- recipe(Response ~ ., treinamento) %>% # define a receita, com a variavel resposta e os dados de treinamento
  step_mutate(Response = as.factor(Response)) %>% 
  step_normalize(all_of(c("MntTotal", "MntRegularProds", "MntWines", 
                                  "MntFruits", "MntMeatProducts", "MntFishProducts", 
                                  "MntSweetProducts", "MntGoldProds", "NumDealsPurchases",
                                  "NumCatalogPurchases", "NumStorePurchases", "NumWebPurchases")))  # normaliza as variaveis não binárias com maior número de outliers
# transformando variável resposta em fator

```

```{r}
receita_prep <- prep(receita) # prepara a receita definida acima

treinamento_proc <- bake(receita_prep, new_data = NULL) # obtem os dados de treinamento processados

teste_proc <- bake(receita_prep, new_data = teste) # obtem os dados de teste processados
```

## 3.1. Modelos Supervisionados

```{r}
# Convertendo 'Response' para fator nos conjuntos de dados
treinamento_proc$Response <- factor(treinamento_proc$Response, levels = c("0", "1"))
teste_proc$Response <- factor(teste_proc$Response, levels = c("0", "1"))

```


```{r}
resultados_modelos <- tibble(
  modelo = character(),
  acuracia = numeric(),
  auc = numeric(),
  sensibilidade = numeric(),
  sensitividade = numeric(),
  verdposit = numeric(),
  verdneg = numeric()
)
```

# 3.1.1. Regressão Logística

Definindo modelo, engine e realizando o fit

```{r}
fit_glm <- logistic_reg() %>% 
  set_engine("glm") %>% 
  set_mode("classification") %>% 
  fit(Response ~ ., treinamento_proc) 


tidy(fit_glm) # estimativas do modelo ajustado em formato tidy

```

Criando Tibble com resultado

```{r}
# Cálculo de métricas para o modelo de regressão logística
fitted_glm <- fit_glm %>%
  predict(new_data = teste_proc, type = "prob") %>% # realiza predicao para os dados de teste
  bind_cols(teste_proc %>% select(Response)) %>% 
  mutate(
    observado_bin = as.factor(Response),  # cria uma coluna com o valor observado de default
    pred_bin = as.factor(if_else(.pred_1 > 0.25, "1", "0"))
  )
```


```{r}
# Calculando as métricas
acc_glm <- accuracy(fitted_glm, truth = observado_bin, estimate = pred_bin, event_level = "second")
auc_glm <- roc_auc(fitted_glm, truth = observado_bin, .pred_1)
sens_glm <- sens(fitted_glm, truth = observado_bin, estimate = pred_bin, event_level = "second")
spec_glm <- spec(fitted_glm, truth = observado_bin, estimate = pred_bin, event_level = "second")
ppv_glm <- ppv(fitted_glm, truth = observado_bin, estimate = pred_bin, event_level = "second")
npv_glm <- npv(fitted_glm, truth = observado_bin, estimate = pred_bin, event_level = "second")

```

```{r}
# Adicionando ao dataframe de resultados
resultados_modelos <- resultados_modelos %>% 
  bind_rows(tibble(
    modelo = "Regressão Logística",
    acuracia = acc_glm$.estimate,
    auc = auc_glm$.estimate,
    sensibilidade = sens_glm$.estimate,
    sensitividade = spec_glm$.estimate,
    verdposit = ppv_glm$.estimate,
    verdneg = npv_glm$.estimate
  ))
resultados_modelos
```
```{r}
confusion_matrix <- confusionMatrix(data = fitted_glm$pred_bin, reference = fitted_glm$observado_bin, positive = "1")

# Imprimindo a matriz de confusão
confusion_matrix
```

### 2.1.2. LASSO

Criando o modelo

```{r}
lasso <- logistic_reg(penalty = tune(), mixture = 1) %>% # define o modelo lasso e o parametro a ser tunado (o lambda)
  set_engine("glmnet") %>% # define a engine do modelo
  set_mode("classification") # define que e'  problema de classificacao
```

Validação Cruzada

```{r}
set.seed(321)

cv_split <- vfold_cv(treinamento, v = 10, strata = "Response")

doParallel::registerDoParallel() # paraleliza os proximos comandos

lambda_tune <- tune_grid(lasso, # especificacao do modelo
                         receita,# a receita a ser aplicada a cada lote
                         resamples = cv_split, # os lotes da validacao cruzada
                         grid = 30,# quantas combinacoes de parametros vamos considerar
                         metrics = metric_set(roc_auc, sens)) # metricas consideradas

autoplot(lambda_tune) # plota os resultados

```

```{r}
lambda_tune %>% 
  collect_metrics() # obtem as metricas calculadas

best <- lambda_tune %>% 
  select_best("roc_auc") # seleciona a melhor combinacao de hiperparametros

fit_lasso <- finalize_model(lasso, parameters = best) %>% # informa os valores de hiperparametros a serem considerados
  fit(Response ~ .,data = treinamento_proc) # executa o modelo com os valores de hiperparametros definidos acima
```

```{r}
fitted_lasso <- fit_lasso %>%
  predict(new_data = teste_proc, type = "prob") %>% 
  bind_cols(teste_proc %>% select(Response)) %>% 
  mutate(
    observado_bin = as.factor(Response),  # cria uma coluna com o valor observado
    pred_bin = as.factor(if_else(.pred_1 > 0.5, "1", "0"))  # cria uma coluna para as previsões
  )
```

```{r}
# Calculando as métricas para LASSO
acc_lasso <- accuracy(fitted_lasso, truth = observado_bin, estimate = pred_bin, event_level = "second")
auc_lasso <- roc_auc(fitted_lasso, truth = observado_bin, .pred_1)
sens_lasso <- sens(fitted_lasso, truth = observado_bin, estimate = pred_bin, event_level = "second")
spec_lasso <- spec(fitted_lasso, truth = observado_bin, estimate = pred_bin, event_level = "second")
ppv_lasso <- ppv(fitted_lasso, truth = observado_bin, estimate = pred_bin, event_level = "second")
npv_lasso <- npv(fitted_lasso, truth = observado_bin, estimate = pred_bin, event_level = "second")

```

```{r}
# Adicionando os resultados do LASSO ao dataframe de resultados
resultados_modelos <- resultados_modelos %>% 
  bind_rows(tibble(
    modelo = "LASSO",
    acuracia = acc_lasso$.estimate,
    auc = auc_lasso$.estimate,
    sensibilidade = sens_lasso$.estimate,
    sensitividade = spec_lasso$.estimate,
    verdposit = ppv_lasso$.estimate,
    verdneg = npv_lasso$.estimate,

  ))
resultados_modelos

```

### 2.1.3. Ridge

Definindo o modelo, parãmetro de tunagem e engine

```{r}
ridge <- logistic_reg(penalty = tune(), mixture = 0) %>%
  set_engine("glmnet") %>%
  set_mode("classification") 
```

Validação Cruzada

```{r}
set.seed(321)

cv_split <- vfold_cv(treinamento, v = 10, strata = "Response")

doParallel::registerDoParallel() # paraleliza os proximos comandos

lambda_tune <- tune_grid(ridge, # especificacao do modelo
                         receita,# a receita a ser aplicada a cada lote
                         resamples = cv_split, # os lotes da validacao cruzada
                         grid = 30,# quantas combinacoes de parametros vamos considerar
                         metrics = metric_set(roc_auc, sens)) # metricas consideradas

autoplot(lambda_tune) # plota os resultados

```

```{r}
lambda_tune %>% 
  collect_metrics() # obtem as metricas calculadas

best <- lambda_tune %>% 
  select_best("roc_auc") # seleciona a melhor combinacao de hiperparametros

fit_ridge <- finalize_model(ridge, parameters = best) %>% # informa os valores de hiperparametros a serem considerados
  fit(Response ~ .,data = treinamento_proc) # executa o modelo com os valores de hiperparametros definidos acima

fitted_ridge <- fit_ridge %>% 
  predict(new_data = teste_proc, type = "prob") %>% 
  bind_cols(teste_proc %>% select(Response)) %>% 
  mutate(
    observado_bin = as.factor(Response),  # cria uma coluna com o valor observado
    pred_bin = as.factor(if_else(.pred_1 > 0.2, "1", "0"))  # cria uma coluna para as previsões
  )

head(fitted_ridge)
tail(fitted_ridge)
```

```{r}
# Calculando as métricas para Ridge
acc_ridge <- accuracy(fitted_ridge, truth = observado_bin, estimate = pred_bin, event_level = "second")
auc_ridge <- roc_auc(fitted_ridge, truth = observado_bin, .pred_1)
sens_ridge <- sens(fitted_ridge, truth = observado_bin, estimate = pred_bin, event_level = "second")
spec_ridge <- spec(fitted_ridge, truth = observado_bin, estimate = pred_bin, event_level = "second")
ppv_ridge <- ppv(fitted_ridge, truth = observado_bin, estimate = pred_bin, event_level = "second")
npv_ridge <- npv(fitted_ridge, truth = observado_bin, estimate = pred_bin, event_level = "second")

# Adicionando os resultados do Ridge ao dataframe de resultados
resultados_modelos <- resultados_modelos %>% 
  bind_rows(tibble(
    modelo = "Ridge",
    acuracia = acc_ridge$.estimate,
    auc = auc_ridge$.estimate,
    sensibilidade = sens_ridge$.estimate,
    sensitividade = spec_ridge$.estimate,
    verdposit = ppv_ridge$.estimate,
    verdneg = npv_ridge$.estimate
  ))

resultados_modelos
```

# 3.1.4. Floresta Aleatória

Definindo o modelo, cálculo de importância das variáveis e tipo de previsão

```{r}
rf <- rand_forest() %>%
  set_engine("ranger",
             importance = "permutation") %>%  
  set_mode("classification")

rf
```

Ajuste do modelo

```{r}
rf_fit <- rf %>% 
  fit(Response ~ ., treinamento_proc)
rf_fit
```

Verificando a importância das variaveis para o modelo

```{r}
vip(rf_fit)
```

Inserindo resultados na tabela

```{r}
fitted_rf <- rf_fit %>% 
  predict(new_data = teste_proc, type = 'prob') %>% # realiza predicao para os dados de teste
  mutate(observado = teste_proc$Response, # mesma estrutura do fitted_lm
         modelo = "random forest")
```


Avaliar desempenho

Ajustando Hiperparâmetros

```{r}
rf2 <- rand_forest(mtry = tune(),
                   trees = tune(), # todos argumentos com tune() serao tunados a seguir  
                   min_n = tune()) %>% 
  set_engine("ranger") %>% #
  set_mode("classification") 
  
rf2
```

Realizando Validação Cruzada

```{r}
set.seed(123)
cv_split <- vfold_cv(treinamento, v = 10)

registerDoParallel() # pararaleliza o processo

# para tunar os parametros
rf_grid <- tune_grid(rf2, # especificacao do modelo
                     receita, # a receita a ser aplicada a cada lote
                     resamples = cv_split, # os lotes da validacao cruzada
                     grid = 10, # quantas combinacoes de parametros vamos considerar
                     metrics = metric_set(roc_auc, accuracy))
```

Plotando os resultados

```{r}
autoplot(rf_grid)
```

Selecionando a melhor combinação e hiperparametros e salvando na variável

```{r}
rf_grid %>% 
  collect_metrics() 

best <- rf_grid %>% 
  select_best("roc_auc")
```

Finalizando o modelo

```{r}
rf_fit2 <- finalize_model(rf2, parameters = best) %>% # informa os valores de hiperparametros a serem considerados
  
  fit(Response ~ ., treinamento_proc) # executa o modelo com os valores de hiperparametros definidos acima
```

Previsão para os dados de teste

```{r}
fitted_rf2 <- rf_fit2 %>%
  predict(new_data = teste_proc, type = 'prob') %>%
  bind_cols(teste_proc %>% select(Response)) %>%
  mutate(
    observado_bin = as.factor(Response),
    pred_bin = as.factor(if_else(.pred_1 > 0.25, "1", "0"))
  )

# Calculando as métricas para a Floresta Aleatória
acc_rf2 <- accuracy(fitted_rf2, truth = observado_bin, estimate = pred_bin, event_level = "second")
auc_rf2 <- roc_auc(fitted_rf2, truth = observado_bin, .pred_1)
sens_rf2 <- sens(fitted_rf2, truth = observado_bin, estimate = pred_bin, event_level = "second")
spec_rf2 <- spec(fitted_rf2, truth = observado_bin, estimate = pred_bin, event_level = "second")
ppv_rf2 <- ppv(fitted_rf2, truth = observado_bin, estimate = pred_bin, event_level = "second")
npv_rf2 <- npv(fitted_rf2, truth = observado_bin, estimate = pred_bin, event_level = "second")

# Adicionando os resultados da Floresta Aleatória ao dataframe de resultados
resultados_modelos <- resultados_modelos %>%
  bind_rows(tibble(
    modelo = "Random Forest - Tuned",
    acuracia = acc_rf2$.estimate,
    auc = auc_rf2$.estimate,
    sensibilidade = sens_rf2$.estimate,
    sensitividade = spec_rf2$.estimate,
    verdposit = ppv_rf2$.estimate,
    verdneg = npv_rf2$.estimate
  ))


resultados_modelos
```


Boosting

```{r}
# Definição do modelo de boosting
boost <- boost_tree(mode = "classification") %>%
  set_engine("xgboost")  # define o motor que vai fazer o ajuste do modelo
  #set_mode("classification") # define que é um modelo de classificação

boost
```

```{r}
# Ajuste do modelo aos dados de treinamento processados
boost_fit <- boost %>% 
  fit(Response ~ ., data = treinamento_proc) # ajuste do modelo

```

```{r}
# Importância das variáveis
vip(boost_fit)

```


```{r}
# Predição para os dados de teste
fitted_boost <- boost_fit %>% 
  predict(new_data = teste_proc)

```

```{r}
# Tune - Ajuste de hiperparâmetros

# Modelo XGBoost com hiperparâmetros a serem ajustados
boost2 <- boost_tree(
    trees = tune(),               
    learn_rate = tune(),          
    tree_depth = tune() 
) %>% 
  set_engine("xgboost") %>% 
  set_mode("classification")
boost2

```

```{r}

# Validação cruzada para ajuste de hiperparâmetros
set.seed(123)
cv_split <- vfold_cv(treinamento, v = 10)  # validação cruzada

registerDoParallel() # Parallelize the process

# Definição do grid de hiperparâmetros
hyper_grid <- grid_regular(
  trees(),
  tree_depth(),
  learn_rate(),
  levels = 4
)

# Para tunar os hiperparâmetros
boost_grid <- tune_grid(
  boost2,
  receita,
  resamples = cv_split,
  grid = hyper_grid,  # número de combinações de parâmetros
  metrics = metric_set(roc_auc, sens, spec)  # métricas para avaliação
)

autoplot(boost_grid) # Visualize the results

```

```{r}
# Encontrar o ponto de corte ótimo
#coords(roc_curve, "best", ret="threshold")

# Ajustar o ponto de corte com base no valor encontrado
#novo_ponto_corte <- coords(roc_curve, "best", ret="threshold")
#fitted_boost_final$pred_bin <- as.factor(if_else(fitted_boost_final$.pred_1 > novo_ponto_corte, "1", "0"))
```

```{r}

boost_grid %>% 
  collect_metrics()

# Melhor conjunto de parâmetros
best <- boost_grid %>% 
  select_best("roc_auc")  # seleciona o melhor modelo baseado no sens

best

```

```{r}
# Finalizando o modelo com os melhores parâmetros
boost_fit_final <- finalize_model(boost2, parameters = best) %>%
  fit(Response ~ ., data = treinamento_proc)

# Predições com o modelo final
fitted_boost_final <- boost_fit_final %>%
  predict(new_data = teste_proc, type = "prob") %>%
  bind_cols(teste_proc %>% select(Response)) %>%
  mutate(
    observado_bin = as.factor(Response),
    pred_bin = as.factor(if_else(.pred_1 > 0.18, "1", "0"))  # Converte probabilidades em classificações binárias
  )

# Garantindo que 'pred_bin' tenha os mesmos níveis que 'observado_bin'
fitted_boost_final$pred_bin <- factor(fitted_boost_final$pred_bin, levels = levels(fitted_boost_final$observado_bin))

# Calculando as métricas para o modelo de Boosting
acc_boost_final <- accuracy(fitted_boost_final, truth = observado_bin, estimate = pred_bin, event_level = "second")
auc_boost_final <- roc_auc(fitted_boost_final, truth = observado_bin, .pred_1)
sens_boost_final <- sens(fitted_boost_final, truth = observado_bin, estimate = pred_bin, event_level = "second")
spec_boost_final <- spec(fitted_boost_final, truth = observado_bin, estimate = pred_bin, event_level = "second")
ppv_boost_final <- ppv(fitted_boost_final, truth = observado_bin, estimate = pred_bin, event_level = "second")
npv_boost_final <- npv(fitted_boost_final, truth = observado_bin, estimate = pred_bin, event_level = "second")


# Adicionando os resultados do modelo de Boosting ao dataframe de resultados
resultados_modelos <- resultados_modelos %>%
  bind_rows(tibble(
    modelo = "Boosting",
    acuracia = acc_boost_final$.estimate,
    auc = auc_boost_final$.estimate,
    sensibilidade = sens_boost_final$.estimate,
    sensitividade = spec_boost_final$.estimate,
    verdposit = ppv_boost_final$.estimate,
    verdneg = npv_boost_final$.estimate
  ))

# Imprimindo os resultados atualizados
resultados_modelos

```
```{r}
confusionMatrix(data = fitted_boost_final$pred_bin, reference = fitted_boost_final$observado_bin)
```

Redes neurais

```{r}

# Preparando dados

# Selecionando variáveis explicativas do treinamento e convertendo dados para matriz
#X_trn <- treinamento %>% 
#  select(-Response) %>% 
#  as.matrix()

X_trn <-treinamento %>%
  mutate(across(c("MntTotal", "MntRegularProds", "MntWines", 
                                  "MntFruits", "MntMeatProducts", "MntFishProducts", 
                                  "MntSweetProducts", "MntGoldProds", "NumDealsPurchases",
                                  "NumCatalogPurchases", "NumStorePurchases", "NumWebPurchases"), scale)) %>% 
  select(c("MntTotal", "MntRegularProds", "MntWines", 
                                  "MntFruits", "MntMeatProducts", "MntFishProducts", 
                                  "MntSweetProducts", "MntGoldProds", "NumDealsPurchases",
                                  "NumCatalogPurchases", "NumStorePurchases", "NumWebPurchases", "Response")) %>% 
  select(-Response) %>% 
  as.matrix()
```

```{r}
# Selecionando variáveis explicativas do teste e convertendo dados para matriz
#X_tst <- teste %>% 
#  select(-Response) %>% 
#  as.matrix()
```

```{r}

# Padroniza os dados de treinamento

X_tst <-teste %>%
  mutate(across(c("MntTotal", "MntRegularProds", "MntWines", 
                                  "MntFruits", "MntMeatProducts", "MntFishProducts", 
                                  "MntSweetProducts", "MntGoldProds", "NumDealsPurchases",
                                  "NumCatalogPurchases", "NumStorePurchases", "NumWebPurchases"), scale)) %>% 
  select(c("MntTotal", "MntRegularProds", "MntWines", 
                                  "MntFruits", "MntMeatProducts", "MntFishProducts", 
                                  "MntSweetProducts", "MntGoldProds", "NumDealsPurchases",
                                  "NumCatalogPurchases", "NumStorePurchases", "NumWebPurchases", "Response")) %>% 
  select(-Response) %>% 
  as.matrix()


# Padroniza os dados de teste usando os mesmos parâmetros de centralização e escala dos dados de treinamento
#X_tst <- scale(X_tst,
#               center = attr(X_trn, "scaled:center"),
#               scale = attr(X_trn, "scaled:scale"))

# Cria um modelo sequencial
net <- keras_model_sequential() %>% 
    layer_dense(units = 128, activation = "relu", input_shape = ncol(X_trn)) %>% # número de características de entrada
    layer_dropout(rate=0.2) %>%  
    layer_dense(units = 64, activation = "relu") %>%
    layer_dropout(rate=0.1) %>% 
    layer_dense(units = 32, activation = "relu") %>%
    #layer_dropout(rate=0.3) %>%  
    layer_dense(units = 8, activation = "relu") %>%
    layer_dense(units = 1, activation = "sigmoid") 

```

```{r}
# Modelo para classificação
net <- compile(net, 
               loss = "binary_crossentropy", 
               optimizer = "rmsprop", 
               metrics = c("Accuracy"))


# Treino dwo modelo para classificação
history_class <- fit(
  net, X_trn, treinamento$Response, 
  batch_size = 16, epochs = 20, 
  validation_split = 0.2)


```

Avaliando o modelo

```{r}
# Fazendo previsões no conjunto de teste
y_pred_prob <- predict(net, X_tst)

# Convertendo as probabilidades de previsão para classificações binárias
y_pred_class <- ifelse(y_pred_prob > min(y_pred_prob), 1, 0)

# Calculando o erro
error_rate <- mean(y_pred_class != teste$Response)

conf_mat <- confusionMatrix(data = as.factor(y_pred_class), reference = as.factor(teste$Response))

# Calculando sensibilidade e especificidade diretamente
sensitivity <- caret::sensitivity(as.factor(y_pred_class), as.factor(teste$Response))
specificity <- caret::specificity(as.factor(y_pred_class), as.factor(teste$Response))
accuracy <- sum(diag(conf_mat$table)) / sum(conf_mat$table)

# Exibindo os resultados
print(paste("Erro: ", error_rate))
print(paste("Sensibilidade: ", sensitivity))
print(paste("Especificidade: ", specificity))
print(paste("Acurácia: ", accuracy))

```

```{r}
conf_mat

```

Comparando desempenho dos modelos

```{r}


fitfull <- fitted %>%
  mutate(
    observado_bin = as.factor(as.numeric(observado) - 1),# Converte o fator para numérico (0 e 1)
    pred_binf = as.factor(if_else(.pred_1 > 0.5, 1, 0)),
    pred_bin = if_else(.pred_1 > 0.5, 1, 0)    # Cria previsões binárias
  ) %>%
  group_by(modelo)


sensib <- fitfull%>% sens(observado_bin, pred_binf) %>% select(.estimate) %>% 
  pull(.estimate)

sensiv <- fitfull %>% spec(observado_bin, pred_binf) %>% select(.estimate)%>% 
  pull(.estimate)
posit <- fitfull  %>% ppv(observado_bin, pred_binf) %>% select(.estimate)%>% 
  pull(.estimate)
negativ <- fitfull  %>% npv(observado_bin, pred_binf) %>% select(.estimate)%>% 
  pull(.estimate)

fitfull %>% roc_auc(observado_bin, .pred_1)  %>% 
  mutate(auc = .estimate,
    sensibilidade= sensib,
        sensitividade = sensiv,
       verdposit = posit,
       verdneg = negativ) %>% select(modelo, auc, sensibilidade, sensitividade, verdposit, verdneg)
 

```


## 3.2. Modelos não Supervisionados

### 3.2.1. KMeans

Primeiramente é realizado o método do cotovelo para identificar o numero ótimo de clusters à serem utilizados

```{r}
set.seed(42)

k <- 2:10
tibble(k = k) %>% 
  mutate(w = map_dbl(k, ~ kmeans(dados, centers = .x,
                            nstart = 10)$tot.withinss)) %>%
  ggplot(aes(k, w)) + 
  geom_point() +
  scale_x_continuous(breaks = k) +
  geom_line()

```
Pelo método do cotovelo é visto que o valor de 5 é um ótimo do modelo, então é analisado esse cluster para a base de dados.

```{r}
set.seed(42)

# Modelo com 5 clusters
k_medias_5 <- kmeans(dados, 
                    centers = 5,
                    nstart = 10)

dados_cluster_5 <- tibble(cluster = k_medias_5$cluster) %>% 
  bind_cols(as_tibble(dados))

head(dados_cluster_5)
```
Com os clusters criados, vai ser analisado a média de valores de alguma coluna para entender os tipos de clientes que foram identificados, assim como a contagem de clientes em cada cluster.

```{r}
# Calcular médias e contar linhas por cluster
summary_stats <- dados_cluster_5 %>% 
  group_by(cluster) %>% 
  summarise(across(starts_with(c('Income', 'Kid', 'Teen', 'Mnt','Num', 'AcceptedCmpOverall')),
                   list(mean = mean)),
            count = n()) %>% 
  rename_all(
    ~str_remove_all(., paste(c("_mean", "Mnt", "Products", "Num", "Cmp"), collapse = "|"))
    )

heatmap(as.matrix(
  summary_stats %>% 
    select(-c("cluster"))), 
  Colv = NA, Rowv = NA, scale = "column", cexCol = .65)

```
Com essa visualização é possível identificar alguns insights relacionado à escolha de clusters vindo do KMeans:

- O cluster 1 apresenta clientes com uma renda menor quando comparado com outros clusters e representam uma parcela menor da base, além de não consumir tantos produtos no aplicativo, porém ainda acessa o site com frequência no mês. Além disso, são clientes que mais tem crianças em casa.

- O cluster 2 são clientes que já tem uma renda levemente maior, e utilizam mais o aplicativos para compras, porém ainda não tem uma conversão completa. Diferentemente do primeiro cluster, esse segmento de clientes apresenta um valor maior de adolescentes em casa do que crianças, o que pode impactar a utilização de aplicativos. Eles representam uma parcela considerável de clientes da base.

- O cluster 3 são clientes que tem uma utilização bem maior quando comparado comm o primeiro e terceiro clusters, além de ter uma renda maior. São clientes mais convertidos, que tem mais probabilidade de aderir a uma campanha e representam uma quantidade significativa na base.

- O cluster 4 se assemelha muito com o primeiro cluster, porém são clientes com uma renda levemente maior e com mais frequência de adolescentes na casa e apresentam um consumo levemente maior comparado com o cluster 1. Eles são o cluster que mais apresentam clientes.

- O cluster 5 seria de clientes que apresentam a maior renda e consequentemente com o maior ticket médio do aplicativo, e são mais engajados em campanhas de marketing, logo podem ser considerados como clientes totalmente convertidos.


Com esses insights, o time de marketing pode adotar algumas estratégias para a próxima campanha de marketing, por exemplo:

- Aumentar o ticket médio de clientes do cluster 5, oferecendo produtos com um valores mais alto, com om intuito de aumentar os lucros da empresa e atingir o break-even do investimento da campanha mais rápido

- Converter clientes do cluster 3, oferecendo planos de fidelidade para estimular a utilização mais recorrente do aplicativo e, consequentemente, aumentando o consumo de produtos no aplicativo.

- Aumentar o interesse de clientes do cluster 1, com cupons de desconto recorrentes no mês para o consumo geral no aplicativo.
